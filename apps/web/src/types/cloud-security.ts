export type CloudSecurityStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface CloudSecurityRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: CloudSecurityStatus; source: string; tags: string[]; assignee?: string;
...
export interface CloudSecurityFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
