...
soc_query_engine_db_t *soc_query_engine_db_create(size_t n) {
  if (!n) n = SOC_QUERY_ENGINE_MAX;
  soc_query_engine_db_t *db = calloc(1, sizeof(*db)); if (!db) return NULL;
  db->items = calloc(n, sizeof(*db->items)); if (!db->items) { free(db); return NULL; }
  db->capacity = n; db->seq = 1; return db; }

void soc_query_engine_db_destroy(soc_query_engine_db_t *db) { if (!db) return; free(db->items); free(db); }

int soc_query_engine_db_upsert(soc_query_engine_db_t *db, const soc_query_engine_rec_t *rec) {
  if (!db || !soc_query_engine_validate(rec)) return -1;
...
void soc_query_engine_metrics(const soc_query_engine_db_t *db, char *buf, size_t len) {
  ...
  snprintf(buf, len, "{\"module\":\"query_engine\",\"service\":\"analytics\",\"count\":%zu,\"cap\":%zu,\"seq\":%llu}",
...
int soc_query_engine_fn_000(soc_query_engine_db_t *db, uint64_t a, const void *b, size_t blen) {
  ...
  if (b && blen) acc ^= soc_query_engine_hash(b, blen);
  for (size_t i=0;i<db->count;i++) {
