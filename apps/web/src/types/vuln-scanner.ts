export type VulnScannerStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface VulnScannerRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: VulnScannerStatus; source: string; tags: string[]; assignee?: string;
...
export interface VulnScannerFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
export interface VulnScannerMetrics {
