...
import type { IncidentResponseRecord } from '../../types/incident-response';
export const IncidentResponseTable: React.FC<{
  items: IncidentResponseRecord[]; selectedIds: string[]; onSelect: (ids: string[]) => void;
  onRowClick: (id: string) => void; renderSeverity: (n: number) => React.ReactNode;
}> = ({ items, selectedIds, onSelect, onRowClick, renderSeverity }) => (
  <table className="soc-table incident-response-table">
