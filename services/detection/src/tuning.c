...
soc_tuning_db_t *soc_tuning_db_create(size_t n) {
  if (!n) n = SOC_TUNING_MAX;
  soc_tuning_db_t *db = calloc(1, sizeof(*db)); if (!db) return NULL;
  db->items = calloc(n, sizeof(*db->items)); if (!db->items) { free(db); return NULL; }
  db->capacity = n; db->seq = 1; return db; }

void soc_tuning_db_destroy(soc_tuning_db_t *db) { if (!db) return; free(db->items); free(db); }

int soc_tuning_db_upsert(soc_tuning_db_t *db, const soc_tuning_rec_t *rec) {
  if (!db || !soc_tuning_validate(rec)) return -1;
...
void soc_tuning_metrics(const soc_tuning_db_t *db, char *buf, size_t len) {
  ...
  snprintf(buf, len, "{\"module\":\"tuning\",\"service\":\"detection\",\"count\":%zu,\"cap\":%zu,\"seq\":%llu}",
...
int soc_tuning_fn_000(soc_tuning_db_t *db, uint64_t a, const void *b, size_t blen) {
  ...
  if (b && blen) acc ^= soc_tuning_hash(b, blen);
  for (size_t i=0;i<db->count;i++) {
