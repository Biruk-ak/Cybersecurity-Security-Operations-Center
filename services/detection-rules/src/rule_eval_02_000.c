...
soc_rule_eval_02_000_db_t *soc_rule_eval_02_000_db_create(size_t n) {
  if (!n) n = SOC_RULE_EVAL_02_000_MAX;
  soc_rule_eval_02_000_db_t *db = calloc(1, sizeof(*db)); if (!db) return NULL;
  db->items = calloc(n, sizeof(*db->items)); if (!db->items) { free(db); return NULL; }
  db->capacity = n; db->seq = 1; return db; }

void soc_rule_eval_02_000_db_destroy(soc_rule_eval_02_000_db_t *db) { if (!db) return; free(db->items); free(db); }

int soc_rule_eval_02_000_db_upsert(soc_rule_eval_02_000_db_t *db, const soc_rule_eval_02_000_rec_t *rec) {
  if (!db || !soc_rule_eval_02_000_validate(rec)) return -1;
...
void soc_rule_eval_02_000_metrics(const soc_rule_eval_02_000_db_t *db, char *buf, size_t len) {
  ...
  snprintf(buf, len, "{\"module\":\"rule_eval_02_000\",\"service\":\"detection-rules\",\"count\":%zu,\"cap\":%zu,\"seq\":%llu}",
...
int soc_rule_eval_02_000_fn_000(soc_rule_eval_02_000_db_t *db, uint64_t a, const void *b, size_t blen) {
  ...
  if (b && blen) acc ^= soc_rule_eval_02_000_hash(b, blen);
  for (size_t i=0;i<db->count;i++) {
