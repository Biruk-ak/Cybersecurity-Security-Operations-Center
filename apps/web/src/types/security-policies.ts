export type SecurityPoliciesStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface SecurityPoliciesRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: SecurityPoliciesStatus; source: string; tags: string[]; assignee?: string;
...
export interface SecurityPoliciesFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
export interface SecurityPoliciesMetrics {
