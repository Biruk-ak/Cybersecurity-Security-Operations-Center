...
import type { DeviceManagementRecord } from '../../types/device-management';
export const DeviceManagementTable: React.FC<{
  items: DeviceManagementRecord[]; selectedIds: string[]; onSelect: (ids: string[]) => void;
  onRowClick: (id: string) => void; renderSeverity: (n: number) => React.ReactNode;
}> = ({ items, selectedIds, onSelect, onRowClick, renderSeverity }) => (
  <table className="soc-table device-management-table">
