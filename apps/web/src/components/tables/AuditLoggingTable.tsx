...
import type { AuditLoggingRecord } from '../../types/audit-logging';
export const AuditLoggingTable: React.FC<{
  items: AuditLoggingRecord[]; selectedIds: string[]; onSelect: (ids: string[]) => void;
  onRowClick: (id: string) => void; renderSeverity: (n: number) => React.ReactNode;
}> = ({ items, selectedIds, onSelect, onRowClick, renderSeverity }) => (
  <table className="soc-table audit-logging-table">
