export type IamStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface IamRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: IamStatus; source: string; tags: string[]; assignee?: string;
...
export interface IamFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
