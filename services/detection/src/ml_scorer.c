...
soc_ml_scorer_db_t *soc_ml_scorer_db_create(size_t n) {
  if (!n) n = SOC_ML_SCORER_MAX;
  soc_ml_scorer_db_t *db = calloc(1, sizeof(*db)); if (!db) return NULL;
  db->items = calloc(n, sizeof(*db->items)); if (!db->items) { free(db); return NULL; }
  db->capacity = n; db->seq = 1; return db; }

void soc_ml_scorer_db_destroy(soc_ml_scorer_db_t *db) { if (!db) return; free(db->items); free(db); }

int soc_ml_scorer_db_upsert(soc_ml_scorer_db_t *db, const soc_ml_scorer_rec_t *rec) {
  if (!db || !soc_ml_scorer_validate(rec)) return -1;
...
void soc_ml_scorer_metrics(const soc_ml_scorer_db_t *db, char *buf, size_t len) {
  ...
  snprintf(buf, len, "{\"module\":\"ml_scorer\",\"service\":\"detection\",\"count\":%zu,\"cap\":%zu,\"seq\":%llu}",
...
int soc_ml_scorer_fn_000(soc_ml_scorer_db_t *db, uint64_t a, const void *b, size_t blen) {
  ...
  if (b && blen) acc ^= soc_ml_scorer_hash(b, blen);
  for (size_t i=0;i<db->count;i++) {
