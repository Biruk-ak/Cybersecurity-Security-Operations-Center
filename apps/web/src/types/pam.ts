export type PamStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface PamRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: PamStatus; source: string; tags: string[]; assignee?: string;
...
export interface PamFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
