...
import type { DlpRecord } from '../../types/dlp';
export const DlpTable: React.FC<{
  items: DlpRecord[]; selectedIds: string[]; onSelect: (ids: string[]) => void;
  onRowClick: (id: string) => void; renderSeverity: (n: number) => React.ReactNode;
}> = ({ items, selectedIds, onSelect, onRowClick, renderSeverity }) => (
  <table className="soc-table dlp-table">
