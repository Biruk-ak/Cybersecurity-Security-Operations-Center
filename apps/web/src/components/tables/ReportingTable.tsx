...
import type { ReportingRecord } from '../../types/reporting';
export const ReportingTable: React.FC<{
  items: ReportingRecord[]; selectedIds: string[]; onSelect: (ids: string[]) => void;
  onRowClick: (id: string) => void; renderSeverity: (n: number) => React.ReactNode;
}> = ({ items, selectedIds, onSelect, onRowClick, renderSeverity }) => (
  <table className="soc-table reporting-table">
