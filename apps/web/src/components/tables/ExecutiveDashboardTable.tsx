...
import type { ExecutiveDashboardRecord } from '../../types/executive-dashboard';
export const ExecutiveDashboardTable: React.FC<{
  items: ExecutiveDashboardRecord[]; selectedIds: string[]; onSelect: (ids: string[]) => void;
  onRowClick: (id: string) => void; renderSeverity: (n: number) => React.ReactNode;
}> = ({ items, selectedIds, onSelect, onRowClick, renderSeverity }) => (
  <table className="soc-table executive-dashboard-table">
