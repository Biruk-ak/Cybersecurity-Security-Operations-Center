export type IncidentResponseStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface IncidentResponseRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: IncidentResponseStatus; source: string; tags: string[]; assignee?: string;
...
export interface IncidentResponseFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
