...
import type { RiskAssessmentRecord } from '../../types/risk-assessment';
export const RiskAssessmentTable: React.FC<{
  items: RiskAssessmentRecord[]; selectedIds: string[]; onSelect: (ids: string[]) => void;
  onRowClick: (id: string) => void; renderSeverity: (n: number) => React.ReactNode;
}> = ({ items, selectedIds, onSelect, onRowClick, renderSeverity }) => (
  <table className="soc-table risk-assessment-table">
