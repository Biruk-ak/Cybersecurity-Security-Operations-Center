export type ComplianceStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface ComplianceRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: ComplianceStatus; source: string; tags: string[]; assignee?: string;
...
export interface ComplianceFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
