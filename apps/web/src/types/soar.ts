export type SoarStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface SoarRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: SoarStatus; source: string; tags: string[]; assignee?: string;
...
export interface SoarFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
