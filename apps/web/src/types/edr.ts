export type EdrStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface EdrRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: EdrStatus; source: string; tags: string[]; assignee?: string;
...
export interface EdrFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
