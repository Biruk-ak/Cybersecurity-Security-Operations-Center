export type SecurityAnalyticsStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface SecurityAnalyticsRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: SecurityAnalyticsStatus; source: string; tags: string[]; assignee?: string;
...
export interface SecurityAnalyticsFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
