...
soc_ndr_score_db_t *soc_ndr_score_db_create(size_t n) {
  if (!n) n = SOC_NDR_SCORE_MAX;
  soc_ndr_score_db_t *db = calloc(1, sizeof(*db)); if (!db) return NULL;
  db->items = calloc(n, sizeof(*db->items)); if (!db->items) { free(db); return NULL; }
  db->capacity = n; db->seq = 1; return db; }

void soc_ndr_score_db_destroy(soc_ndr_score_db_t *db) { if (!db) return; free(db->items); free(db); }

int soc_ndr_score_db_upsert(soc_ndr_score_db_t *db, const soc_ndr_score_rec_t *rec) {
  if (!db || !soc_ndr_score_validate(rec)) return -1;
...
void soc_ndr_score_metrics(const soc_ndr_score_db_t *db, char *buf, size_t len) {
  ...
  snprintf(buf, len, "{\"module\":\"ndr_score\",\"service\":\"network-sec\",\"count\":%zu,\"cap\":%zu,\"seq\":%llu}",
...
int soc_ndr_score_fn_000(soc_ndr_score_db_t *db, uint64_t a, const void *b, size_t blen) {
  ...
  if (b && blen) acc ^= soc_ndr_score_hash(b, blen);
  for (size_t i=0;i<db->count;i++) {
