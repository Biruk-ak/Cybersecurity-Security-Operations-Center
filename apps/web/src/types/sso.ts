export type SsoStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface SsoRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: SsoStatus; source: string; tags: string[]; assignee?: string;
...
export interface SsoFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
