export type MfaStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface MfaRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: MfaStatus; source: string; tags: string[]; assignee?: string;
...
export interface MfaFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
