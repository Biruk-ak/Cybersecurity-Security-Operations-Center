...
import type { VulnScannerRecord } from '../../types/vuln-scanner';
export const VulnScannerTable: React.FC<{
  items: VulnScannerRecord[]; selectedIds: string[]; onSelect: (ids: string[]) => void;
  onRowClick: (id: string) => void; renderSeverity: (n: number) => React.ReactNode;
}> = ({ items, selectedIds, onSelect, onRowClick, renderSeverity }) => (
  <table className="soc-table vuln-scanner-table">
