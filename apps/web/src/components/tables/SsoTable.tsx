...
import type { SsoRecord } from '../../types/sso';
export const SsoTable: React.FC<{
  items: SsoRecord[]; selectedIds: string[]; onSelect: (ids: string[]) => void;
  onRowClick: (id: string) => void; renderSeverity: (n: number) => React.ReactNode;
}> = ({ items, selectedIds, onSelect, onRowClick, renderSeverity }) => (
  <table className="soc-table sso-table">
