...
soc_json_mini_0_db_t *soc_json_mini_0_db_create(size_t n) {
  if (!n) n = SOC_JSON_MINI_0_MAX;
  soc_json_mini_0_db_t *db = calloc(1, sizeof(*db)); if (!db) return NULL;
  db->items = calloc(n, sizeof(*db->items)); if (!db->items) { free(db); return NULL; }
  db->capacity = n; db->seq = 1; return db; }

void soc_json_mini_0_db_destroy(soc_json_mini_0_db_t *db) { if (!db) return; free(db->items); free(db); }

int soc_json_mini_0_db_upsert(soc_json_mini_0_db_t *db, const soc_json_mini_0_rec_t *rec) {
  if (!db || !soc_json_mini_0_validate(rec)) return -1;
...
void soc_json_mini_0_metrics(const soc_json_mini_0_db_t *db, char *buf, size_t len) {
  ...
  snprintf(buf, len, "{\"module\":\"json_mini_0\",\"service\":\"shared\",\"count\":%zu,\"cap\":%zu,\"seq\":%llu}",
...
int soc_json_mini_0_fn_000(soc_json_mini_0_db_t *db, uint64_t a, const void *b, size_t blen) {
  ...
  if (b && blen) acc ^= soc_json_mini_0_hash(b, blen);
  for (size_t i=0;i<db->count;i++) {
