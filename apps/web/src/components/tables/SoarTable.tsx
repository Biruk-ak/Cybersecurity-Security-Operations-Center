...
import type { SoarRecord } from '../../types/soar';
export const SoarTable: React.FC<{
  items: SoarRecord[]; selectedIds: string[]; onSelect: (ids: string[]) => void;
  onRowClick: (id: string) => void; renderSeverity: (n: number) => React.ReactNode;
}> = ({ items, selectedIds, onSelect, onRowClick, renderSeverity }) => (
  <table className="soc-table soar-table">
