export type AuditLoggingStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface AuditLoggingRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: AuditLoggingStatus; source: string; tags: string[]; assignee?: string;
...
export interface AuditLoggingFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
