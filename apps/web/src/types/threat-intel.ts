export type ThreatIntelStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface ThreatIntelRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: ThreatIntelStatus; source: string; tags: string[]; assignee?: string;
...
export interface ThreatIntelFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
