export type FirewallManagementStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface FirewallManagementRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: FirewallManagementStatus; source: string; tags: string[]; assignee?: string;
...
export interface FirewallManagementFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
export interface FirewallManagementMetrics {
