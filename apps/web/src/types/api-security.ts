export type ApiSecurityStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface ApiSecurityRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: ApiSecurityStatus; source: string; tags: string[]; assignee?: string;
...
export interface ApiSecurityFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
