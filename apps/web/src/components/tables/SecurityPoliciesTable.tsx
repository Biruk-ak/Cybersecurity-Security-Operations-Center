...
import type { SecurityPoliciesRecord } from '../../types/security-policies';
export const SecurityPoliciesTable: React.FC<{
  items: SecurityPoliciesRecord[]; selectedIds: string[]; onSelect: (ids: string[]) => void;
  onRowClick: (id: string) => void; renderSeverity: (n: number) => React.ReactNode;
}> = ({ items, selectedIds, onSelect, onRowClick, renderSeverity }) => (
  <table className="soc-table security-policies-table">
