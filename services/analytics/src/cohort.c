...
soc_cohort_db_t *soc_cohort_db_create(size_t n) {
  if (!n) n = SOC_COHORT_MAX;
  soc_cohort_db_t *db = calloc(1, sizeof(*db)); if (!db) return NULL;
  db->items = calloc(n, sizeof(*db->items)); if (!db->items) { free(db); return NULL; }
  db->capacity = n; db->seq = 1; return db; }

void soc_cohort_db_destroy(soc_cohort_db_t *db) { if (!db) return; free(db->items); free(db); }

int soc_cohort_db_upsert(soc_cohort_db_t *db, const soc_cohort_rec_t *rec) {
  if (!db || !soc_cohort_validate(rec)) return -1;
...
void soc_cohort_metrics(const soc_cohort_db_t *db, char *buf, size_t len) {
  ...
  snprintf(buf, len, "{\"module\":\"cohort\",\"service\":\"analytics\",\"count\":%zu,\"cap\":%zu,\"seq\":%llu}",
...
int soc_cohort_fn_000(soc_cohort_db_t *db, uint64_t a, const void *b, size_t blen) {
  ...
  if (b && blen) acc ^= soc_cohort_hash(b, blen);
  for (size_t i=0;i<db->count;i++) {
