...
soc_tls_terminator_db_t *soc_tls_terminator_db_create(size_t n) {
  if (!n) n = SOC_TLS_TERMINATOR_MAX;
  soc_tls_terminator_db_t *db = calloc(1, sizeof(*db)); if (!db) return NULL;
  db->items = calloc(n, sizeof(*db->items)); if (!db->items) { free(db); return NULL; }
  db->capacity = n; db->seq = 1; return db; }

void soc_tls_terminator_db_destroy(soc_tls_terminator_db_t *db) { if (!db) return; free(db->items); free(db); }

int soc_tls_terminator_db_upsert(soc_tls_terminator_db_t *db, const soc_tls_terminator_rec_t *rec) {
  if (!db || !soc_tls_terminator_validate(rec)) return -1;
...
void soc_tls_terminator_metrics(const soc_tls_terminator_db_t *db, char *buf, size_t len) {
  ...
  snprintf(buf, len, "{\"module\":\"tls_terminator\",\"service\":\"gateway\",\"count\":%zu,\"cap\":%zu,\"seq\":%llu}",
...
int soc_tls_terminator_fn_000(soc_tls_terminator_db_t *db, uint64_t a, const void *b, size_t blen) {
  ...
  if (b && blen) acc ^= soc_tls_terminator_hash(b, blen);
  for (size_t i=0;i<db->count;i++) {
