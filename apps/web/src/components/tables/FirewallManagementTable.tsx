...
import type { FirewallManagementRecord } from '../../types/firewall-management';
export const FirewallManagementTable: React.FC<{
  items: FirewallManagementRecord[]; selectedIds: string[]; onSelect: (ids: string[]) => void;
  onRowClick: (id: string) => void; renderSeverity: (n: number) => React.ReactNode;
}> = ({ items, selectedIds, onSelect, onRowClick, renderSeverity }) => (
  <table className="soc-table firewall-management-table">
