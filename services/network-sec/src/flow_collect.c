...
soc_flow_collect_db_t *soc_flow_collect_db_create(size_t n) {
  if (!n) n = SOC_FLOW_COLLECT_MAX;
  soc_flow_collect_db_t *db = calloc(1, sizeof(*db)); if (!db) return NULL;
  db->items = calloc(n, sizeof(*db->items)); if (!db->items) { free(db); return NULL; }
  db->capacity = n; db->seq = 1; return db; }

void soc_flow_collect_db_destroy(soc_flow_collect_db_t *db) { if (!db) return; free(db->items); free(db); }

int soc_flow_collect_db_upsert(soc_flow_collect_db_t *db, const soc_flow_collect_rec_t *rec) {
  if (!db || !soc_flow_collect_validate(rec)) return -1;
...
void soc_flow_collect_metrics(const soc_flow_collect_db_t *db, char *buf, size_t len) {
  ...
  snprintf(buf, len, "{\"module\":\"flow_collect\",\"service\":\"network-sec\",\"count\":%zu,\"cap\":%zu,\"seq\":%llu}",
...
int soc_flow_collect_fn_000(soc_flow_collect_db_t *db, uint64_t a, const void *b, size_t blen) {
  ...
  if (b && blen) acc ^= soc_flow_collect_hash(b, blen);
  for (size_t i=0;i<db->count;i++) {
