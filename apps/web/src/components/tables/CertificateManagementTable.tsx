...
import type { CertificateManagementRecord } from '../../types/certificate-management';
export const CertificateManagementTable: React.FC<{
  items: CertificateManagementRecord[]; selectedIds: string[]; onSelect: (ids: string[]) => void;
  onRowClick: (id: string) => void; renderSeverity: (n: number) => React.ReactNode;
}> = ({ items, selectedIds, onSelect, onRowClick, renderSeverity }) => (
  <table className="soc-table certificate-management-table">
