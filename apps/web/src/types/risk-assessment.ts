export type RiskAssessmentStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface RiskAssessmentRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: RiskAssessmentStatus; source: string; tags: string[]; assignee?: string;
...
export interface RiskAssessmentFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
