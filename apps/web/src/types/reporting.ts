export type ReportingStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface ReportingRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: ReportingStatus; source: string; tags: string[]; assignee?: string;
...
export interface ReportingFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
