export type EncryptionStatus = 'open'|'investigating'|'resolved'|'closed'|'suppressed';
export interface EncryptionRecord {
  id: string; tenantId: string; name: string; description: string; severity: number;
  status: EncryptionStatus; source: string; tags: string[]; assignee?: string;
...
export interface EncryptionFilter { severityMin: number; query: string; status: string; page: number; pageSize: number; }
